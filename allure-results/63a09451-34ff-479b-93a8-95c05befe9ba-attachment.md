...
```ts
  ...
  28 |     } else {
> 29 |       await expect(page).not.toHaveURL(/overview/, { timeout: 60000 });
     |                              ^ Error: expect(page).not.toHaveURL(expected) failed
  30 |     }
  31 | 
  32 |   });
