...
Locator:  locator('h1.title').filter({ hasText: /Error/i })
Expected: visible
Received: hidden

Call log:
  - Expect "toBeVisible" with timeout 60000ms
  - waiting for locator('h1.title').filter({ hasText: /Error/i })
    24 × locator resolved to <h1 class="title">↵⇆⇆⇆Error!↵⇆⇆</h1>
...
```ts
  ...
  28 |     } else {
> 29 |       await expect(page.locator('h1.title', { hasText: /Error/i })).toBeVisible({ timeout: 60000 });
     |                                                                     ^ Error: expect(locator).toBeVisible() failed
  30 |     }
  31 | 
  32 |   });
